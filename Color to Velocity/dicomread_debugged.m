%   NOTE: THIS FUNCTION HAS BEEN MODIFIED FROM ITS ORIGINAL VERSION WTHIN
%   MATLAB IMAGE PROCESSING TOOLBOX.
%   Edits at lines 939 and 1480
%
%
% Edited by: Denis Routkevitch (droutke1@jhmi.edu)
% Last edited: 1/24/2021

function [X, map, alpha, overlays] = dicomread_debugged(msgname, varargin)
%DICOMREAD  Read DICOM image.
%   X = DICOMREAD(FILENAME) reads the image data from the compliant DICOM
%   or DICOS file FILENAME.  For single-frame grayscale images, X is an
%   M-by-N array.  For single-frame true-color images, X is an M-by-N-by-3
%   array. Multiframe images are always 4-D arrays.
%
%   X = DICOMREAD(INFO) reads the image data from the message referenced in
%   the DICOM or DICOS metadata structure INFO.  The INFO structure is
%   produced by the DICOMINFO function.
%
...
%   from the DICOM or DICOS file.  Each overlay is a 1-bit black and white
%   image with the same height and width as X.  If multiple overlays are
%   present in the file, OVERLAYS is a 4-D multiframe image.  If no
%   overlays are in the file, OVERLAYS is empty.
%
...
if (metadata.NumberOfFrames == 1)
    ...
    %%%% The following for loop has been edited by Denis Routkevitch %%%%
    for ii = 1:length(frameList)
        ...
        % it back out.
        frameStart = offset + offsetTable(ii);
...
for ii = 1:length(frames)
    ...
    if (ii ~= numel(offsetTable))
        
        lastBase = frameLocation;
        lastFrameLength = offsetTable(ii+1) - offsetTable(ii);
    end

    offsetTable(ii) = frameLocation;
