...
function [pre_values, post_values] = find_injury_regions(avpre, avpost, time, dist, prom, filt_size)
    ...
    post_values = struct('InjuryHeight',[], ...
                         'CranPenHeight',[], ...
                         'CaudPenHeight',[], ...
                         'CranNormHeight',[], ...
                         'CaudNormHeight',[], ...
    ...
    % for generating pre-injury time series
...
%     transition(1:step,2) = transition(step+1,2);
    ...
    % pre values time series instead
%     pre_values.InjuryHeight = (avpre(:,umbra_loc(1)));
%     pre_values.CranPenHeight = (avpre(:,cran_loc(1)));
%     pre_values.CaudPenHeight = (avpre(:,caud_loc(1)));
%     pre_values.CranNormHeight = (crannormpre);
%     pre_values.CaudNormHeight = (caudnormpre);
